...
MzResult mz_migrate_pages(struct page *page)
{
	...
	// migrate_prep(); // Removed - obsolete in new kernels

	// Use __isolate_lru_page(page, ISOLATE_UNMAPPED) for modern kernels
	res = __isolate_lru_page(page, ISOLATE_UNMAPPED);
	if (res < 0) {
		MZ_LOG(err_level_error, "%s __isolate_lru_page fail %d\n", __func__, res);
		return MZ_PAGE_FAIL;
	}
							...
							struct page **target_page, int after_mig, uint8_t __user *buf)
{
	...
	// Updated for new kernel API: get_user_pages_remote now usually takes 7 arguments, and omits task
							...
							unsigned long *new_pfn, pid_t tgid)
{
	...
	/*
	 * Use 'mm->mmap_lock' instead of 'mm->mmap_sem' for recent kernels.
...
unsigned long mz_ptw(unsigned long ava, struct mm_struct *mm)
{
	...
	// Insert p4d_offset step for new kernel API
